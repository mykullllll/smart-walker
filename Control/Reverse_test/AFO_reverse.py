...
class main_loop:
    def __init__(self,fs=10, wheel_radius=0.1143):
        ...
        self.reverse_accel = 0.10
        self.reverse_delta_v = (
            self.reverse_accel / self.fs
        ) / self.wheel_radius
        self.current_velocity = 0
        self.cadence = None
        self.prev_cadence = 1.0

        self.reverse_enter = -0.75
        self.reverse_exit = -0.65
        self.reverse_speed = -0.05
    ...
    def step_from_legs(self, current_time, encoder_velocity, left_x, right_x, isoccluded):
        if left_x is not None and right_x is not None and encoder_velocity is not None:
            ...
            freeze_detected_reverse = False

            monitor_reverse_stationary = (
                self.reverse
                or pelvis < self.reverse_enter)

            if monitor_reverse_stationary and not isoccluded:
                raw_scissor = left - right
                self.stationary_history.append(
                    (current_time, raw_scissor)
                )

                while (
                    self.stationary_history
                    and current_time - self.stationary_history[0][0]
                    > self.freeze_window_duration
                ):
                    self.stationary_history.popleft()

                window_age = (
                    current_time - self.stationary_history[0][0]
                )
                motion_range_reverse = np.ptp(
                    [value for _, value in self.stationary_history]
                )

                freeze_detected_reverse = (
                    window_age >= self.freeze_detection_time
                    and motion_range_reverse
                    < self.freeze_motion_threshold
                )
            else:
            ...
            if isoccluded:
                ...
                if (
                    pelvis >= self.reverse_exit
                    or not freeze_detected_reverse
                ):
                    self.reverse = False
                    linear_velocity = 0.0
                    self.control_state.append(4)
                else:
                    linear_velocity = self.reverse_speed
                    self.control_state.append(5)

            elif freeze_detected:
                linear_velocity = 0.0
                self.control_state.append(4)

            elif pelvis < self.reverse_enter:
                linear_velocity = 0.0

                if (
                    freeze_detected_reverse
                    and abs(encoder_velocity) < 0.10
                ):
                    self.reverse = True

                self.control_state.append(4)

            elif -0.60 < pelvis < -0.50:
                linear_velocity = attenuation(
                    pelvis, -0.60, -0.50
            ...
            if self.reverse is False:
                ...
                target_wheel_velocity = np.clip(target_wheel_velocity, 0, 0.684 / self.wheel_radius) #Cap velocity at 3 m/s

            elif self.reverse is True:
                target_wheel_velocity = linear_velocity / self.wheel_radius
                target_wheel_velocity = np.clip(target_wheel_velocity, self.reverse_speed / self.wheel_radius, 0) #Cap velocity at 3 m/s

            if self.control_state[-1] == 5:
                delta_limit = self.reverse_delta_v
            elif self.control_state[-1] == 4:
